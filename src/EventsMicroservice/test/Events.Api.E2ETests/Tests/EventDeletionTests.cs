...
public class EventDeletionTests(EventsApiFactory factory) : IClassFixture<EventsApiFactory>
{
    ...
    public async Task CreateAndDelete_ShouldNotAffectOtherEvents()
    {
        ...
        var created1 = await response1.Content.ReadFromJsonAsync<EventOverviewDto>(cancellationToken: TestContext.Current.CancellationToken);
        ...
        var created2 = await response2.Content.ReadFromJsonAsync<EventOverviewDto>(cancellationToken: TestContext.Current.CancellationToken);
        ...
        var fetched = await getResponse.Content.ReadFromJsonAsync<EventOverviewDto>(cancellationToken: TestContext.Current.CancellationToken);
